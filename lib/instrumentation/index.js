...
Instrumentation.prototype.start = function () {
  ...
  var customRouterMap = this._agent._conf.customRouterMap

  this._agent.logger.debug('adding hook to Node.js module loader')

  var _MODULES = MODULES.concat(Object.keys(customRouterMap))

  this._hook = hook(_MODULES, function (exports, name, basedir) {
    ...
    if (basedir) {
      ...
      try {
        if (customRouterMap[name]) {
          version = customRouterMap[name].version
          name = customRouterMap[name].name
        } else {
